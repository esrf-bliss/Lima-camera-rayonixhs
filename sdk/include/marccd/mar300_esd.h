...
typedef struct mar_message {
        char            date[10];
        char            time[8];
        short int       task_no;
        short int       error_class;
        short int       error_no;
} MESSAGE;
...
/*
 * ---- Last section =   64 bytes, so far   64 bytes
 *
...
/*
 * ---- Last section =   22 bytes, so far   86 bytes
 *
 ...
 */
	...
	int	expose_phi_speed;	/* 008C      1       *24*   	*/
	int	exp_delta_phi_steps;	/* 0090      1       *25*	*/
	int	search_reference_time;  /* 0094  60000       *26*	*/
	int	leaving_reference_time; /* 0098  15000       *27*	*/
	int	valid_blocks_tolerance; /* 009C      2       *28*	*/
	int	test_level;             /* 00A0     10       *29*	*/
/*
 * ---- Last section =   62 bytes, so far   148 bytes
 */
	int	undefined_2_[7 ];	/*(00A4-00BC)	    *30-36*	*/
/*
 * ---- Last section =   44 bytes, so far   192 bytes
 *
 * ---- Internal step_motor control, etc.:
 */
	short  reserved_1_[8];		/*(00C0=00CE = 16 chars)	*/
/*
 * ---- Last section =   16 bytes, so far   208 bytes
 *
...
/*
 * ---- Last section =   32 bytes, so far   240 bytes
 *
...
/*
 * ---- Last section =   16 bytes, so far   256 bytes
 *
 *	At the end, padded to 2 blocks
 */
	MESSAGE message[32];		/* 0100-03FF */
/*
 * ---- Last section =  768 bytes, so far  1024 bytes
 */
};
